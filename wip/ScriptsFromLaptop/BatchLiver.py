...
if __name__ == "__main__":
    ...
    BASE_DRIVE = r"G:\Shared drives\HEPARIM\Patients 001-029"
    OUTPUT_BASE = r"X:\abdominal_imaging\Shared\HEPARIMTS\batch_processing"

    all_summary_results = []

    for pid in PATIENT_IDS:
        print(f"\n{'='*60}\nSTARTING PROCESSING FOR PATIENT {pid}\n{'='*60}")
    ...
    if all_summary_results:
        ...
        summary_path = os.path.join(OUTPUT_BASE, "global_dice_summary.csv")
        pivot_df = df.pivot(index='Patient_ID', columns='Structure', values='Dice_Score')
        pivot_df.to_csv(summary_path)
        
        print("\n" + "#"*60)
        print(f"BATCH COMPLETE. Results saved to: {summary_path}")
        print("#"*60)