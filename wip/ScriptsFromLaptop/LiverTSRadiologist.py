...
def create_multi_organ_mosaic(image_path, liver_path, aorta_path, portal_path, output_png, num_slices=12):
    ...
    # Use int16 to handle more labels
    combined_mask = np.zeros_like(img_arr, dtype=np.int16)

    # 1. Load Liver Segments (Keep values 1-8 as they are)
    ...
    # 2. Load Aorta (Assign to ID 9)
    if os.path.exists(aorta_path):
        aorta_data = sitk.GetArrayFromImage(sitk.ReadImage(aorta_path))
        combined_mask[aorta_data > 0] = 9

    # 3. Load Portal Vein (Assign to ID 10)
    if os.path.exists(portal_path):
        portal_data = sitk.GetArrayFromImage(sitk.ReadImage(portal_path))
        combined_mask[portal_data > 0] = 10
    ...
    # Use a colormap with enough variety for 10 distinct structures
    # 'tab10' or 'Paired' are good for categorical data
    from matplotlib.cm import get_cmap
    cmap = get_cmap('tab10', 11) 

    for i, idx in enumerate(selected_indices):
        ax = axes[i]
        # Robust contrast scaling for MRI
    ...
    plt.close()
    print(f"✅ Mosaic with 8 Segments + Vessels saved to: {output_png}")        
...
def segment_aorta_segment(input_nifti_path, output_mask_path):
    """
    Runs TotalSegmentator on a NIfTI file and saves the result using SimpleITK
    to ensure coordinate systems match.
    """
...
def segment_portal_vein_segment(input_nifti_path, output_mask_path):
    """
    Runs TotalSegmentator on a NIfTI file and saves the result using SimpleITK
    to ensure coordinate systems match.
    """
    ...
    for pid in PATIENT_IDS:
        try:
            ...
            if convert_dicom_to_nifti(input_dicom, ref_nifti):
                ...
                seg_ok = segment_liver_segments(ref_nifti, ts_mask_liver)
                seg_a = segment_aorta_segment(ref_nifti, ts_mask_aorta)
                seg_p = segment_portal_vein_segment(ref_nifti, ts_mask_portal)
                
                # --- Generate the Mosaic ---
                mosaic_path = os.path.join(patient_out_dir, f"patient_{pid}_combined_mosaic.png")
                create_multi_organ_mosaic(
                    image_path=ref_nifti, 
                    liver_path=ts_mask_liver, 
                    aorta_path=ts_mask_aorta, 
                    portal_path=ts_mask_portal, 
                    output_png=mosaic_path)
                
                # Step C: MITK -> NIfTI
                convert_mitk_suffix_mapping(mitk_input, ref_nifti, manual_mask)
               
                visualize_edit_and_save(ref_nifti, ts_mask_liver)  
                
                #Volume comparison
                if os.path.exists(manual_mask) and os.path.exists(ts_mask_liver):
                        plot_volume_correlation(manual_mask, ts_mask_liver, pid)
        
                # 4. ANALYSIS (Dice Scoring)
                if seg_ok and os.path.exists(manual_mask):
                    dice_metrics = calculate_dice(manual_mask, ts_mask_liver)
